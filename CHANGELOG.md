...
- Влит PR по автоматическому выбору версии KBT (special thanks @@antstas)
- Обновлена логика получения корреляционного события для новой версии KBT при нажатии на кнопки в интеграционных тестах
- Версия SIEMJ теперь запоминается при загрузке плагина и смене KBT чтобы избежать множества лишних запусков SIEMJ

## 3.21.2 (Pre-Release)

- Обновлён тег при оборачивании в конверт, теперь проставляется значение 'vscode_xp'
- Добавлено поле scope_id при оборачивании в конверт сырого события
- Добавлены значения по умолчанию для описания в локализациях правил корреляции и нормализации
- Временно отключена проверка, что описания в локализации не пустые

## 3.21.1 (Pre-Release)

- Для новых версий KBT поддержан метараннер TABLES_SCHEMA, который запускает новую версию утилиты mktables
- Исправлена ошибка при определении версии SIEMJ на Windows
- Во вью с интеграционными тестами добавились дополнительные поля для отображения информации о неуспешном запуске (появляются только с новой версией KBT)
- Исправлена ошибка валидации модульного теста нормализации при использовании новых утилит

## 3.21.0 (Pre-Release)

- Появилась поддержка нескольких версий KBT с переключением через UI
- В настройках появились поля для ввода путей необходимых для нового LSP-сервера
- Автоматическое обновление настроек LSP-сервера при смене версии KBT
- Поддержаны новые утилиты и конфиг для SIEMJ

## 3.20.4 (Release)

- Вернулись к единственной вкладке с модульными тестами из-за кеширования правила при открытии webview
- Добавлено предупреждающее диалоговое окно при попытке открыть модульные тесты другой формулы нормализации
- Исправлена проблема двойного всплывающего окна от Language Server
- Исправлен алгоритм генерации идентификатора правила локализации нормализаций
- Изменения в шаблонах правил
- Обновлена матрица MITRE ATT&CK до 17 версии

## 3.20.3 (Release)

- Исправлено перемещение compositeFields в конец complex_key
- Исправлена логика обработки пустых значений для defaults. Поправлена табуляция с 2 пробелов на 4 для сохранения отступов из PTKB.
- Свойство userCanEditContent теперь есть только у справочников
- Исправлена проблема парсинга теста из issue #219
- Исправлена надпись в редакторе модульных тестов
- Добавлена поддержка ключевого слова any в тестах
- В редакторе модульных тестов: восстановлен функционал по комментированию строк по Ctrl+/, добавлены автозакрывающиеся пары символов и их подсветка
- Контекстные меню для корреляций и обогащений приведены к единому виду
- Обновлены шаблоны правил корреляции
- Добавлены новые источники данных для метаинформации
...
- Добавлена возможность подключения кастомного language-сервера
- Исправлены баги во вьюшке метаинформации
- Исправлена коллизия при распаковке kb-файла (special thanks @qwazzz0)
- Обновлен пайплайн сборки и публикации расширения (special thanks @KleinQuartic)
