...
- снова вернули работу хоткеев `Ctrl+Z` и `Ctrl+Y` во вьюшку интеграционных тестов, которая была времена исключена в версии 3.9.2 (Pre-Release);
- если не удалось получить ожидаемое правило корреляции в интеграционных тестах, то по кнопке `Сравнить полученные результаты` в интеграционных тестах теперь будут все полученные события;
