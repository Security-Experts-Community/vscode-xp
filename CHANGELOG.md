...
- возвращены локаничные описания контекстного меню `Редактор правил локализации` → `Локализации`, `Редактор метаинформации` → `Метаинформация`;
- фича [#128](https://github.com/Security-Experts-Community/vscode-xp/issues/128) - единый порядок элементов контекстного меню для корреляций и нормализаций;
