...
export class PackKbCommand extends ViewCommand {
  constructor(
    ...
    private unpackKbFilePath: string
  ) {
    super();
  }
  ...
  public async execute(): Promise<void> {
    ...
    if (packageContentPrefixRegExp && packageContentPrefixRegExp.length == 2) {
      ...
      if (packageContentPrefix !== currentContentPrefix) {
        DialogHelper.showWarning(
          `Имя поставщика ${currentContentPrefix} не соответствует ObjectId пакета ${packageObjectId}, возможны проблемы при его установке в продукт. Смените имя поставщика или ObjectId пакета`
        );
      }
    } else {
    ...
    return vscode.window.withProgress(
      ...
      async (progress) => {
        try {
          ...
          const objectsPackageDirPath = path.join(
            tmpPackageDirectoryPath,
            ContentTreeProvider.PACKAGES_DIRNAME,
            packageName
          );
          ...
          // Создаем contracts
          const contractsDirPath = path.join(
            tmpPackageDirectoryPath,
