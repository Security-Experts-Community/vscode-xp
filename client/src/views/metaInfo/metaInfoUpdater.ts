...
export class MetaInfoUpdater {
  public update(metaInfo: MetaInfo, fields: any): MetaInfo {
    ...
    if (fields.DataSources) {
      const dataSources = Array.from(fields.DataSources).map((dt) =>
        classTransformer.plainToInstance(DataSource, dt)
      );
      ...
      if (errorDs) {
        throw new IncorrectFieldFillingException(
          `Некорректное заполнение списка событий провайдера '${errorDs.Provider}', который должен быть указаны в виде списка идентификаторов через запятую.`
        );
      }
    ...
    if (fields.ATTACK) {
      const attacks = Array.from(fields.ATTACK).map((dt) =>
        classTransformer.plainToInstance(Attack, dt)
      );
      ...
      for (const attack of attacks) {
        ...
        if (!validationResult) {
          throw new IncorrectFieldFillingException(
            `Некорректное заполнение тактики '${attack.Tactic}'. Техники должны начинаться с 'T' и быть в формате T<N> или T<N>.<M>, где <N>, <M> - натуральные числа.`
          );
        }
      }
