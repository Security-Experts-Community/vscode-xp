...
**Установка xp-kbt внутри контейнера.** Когда wizard скачивает `xp-kbt`, он выполняет внутри контейнера `curl -fL <github-release-asset> | tar/unzip`, а при отсутствии `curl`/`tar`/`unzip` может доустановить их через `apt-get`/`apk`. Это означает доверие к репозиторию [`vxcontrol/xp-kbt`](https://github.com/vxcontrol/xp-kbt/releases) и к TLS-соединению с GitHub, а также требует доступа в сеть и root внутри контейнера. Целостность архива по контрольной сумме сейчас не проверяется. Если это не соответствует вашим политикам безопасности, установите `xp-kbt` в контейнер заранее самостоятельно и укажите путь в `xpConfig.docker.kbtBaseDirectory` вместо автоматической загрузки.
